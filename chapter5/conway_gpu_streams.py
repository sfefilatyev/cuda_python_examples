"""
THIS MODULE IMPLEMENTS STEAMS-CENTERED VERSION OF CONWAY GAME.
...
def update_gpu(frameNum, img, newLattice_gpu, lattice_gpu, N, streams, num_concurrent):
    for k in range(num_concurrent):
        conway_ker(newLattices_gpu[k], lattices_gpu[k], grid=(int(N/32), int(N/32), 1), block=(32,32,1), stream=streams[k])        
...
if __name__ == '__main__':
    ...
    ani = animation.FuncAnimation(fig,
                                  update_gpu,
                                  fargs=(imgs, newLattices_gpu, lattices_gpu, N, streams, num_concurrent),
                                  interval=1,
                                  frames=1000,
                                  save_count=1000)
    plt.show()
